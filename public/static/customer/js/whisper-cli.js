...
if (config !== undefined && config.socket !== undefined) {
    ...
    socket.onopen = function (res) {
        console.log('握手成功');
        // 登录
        let login_data = '{"type":"userInit", "uid": ' + config.uid + ', "name" : "' + config.name +
    ...
    socket.onmessage = function (res) {
        ...
        switch (data['message_type']) {
            ...
            case 'endAsk':
                lockTextarea();
                if ('会诊已结束' === data.data.content) {
                    socket.close();
                }
                document.getElementById('title').innerHTML = '会诊已结束';
                showSystem(data.data);
                break;
            // 监测聊天数据
            case 'chatMessage':
...
function showSystem(msg) {
    let _html = document.getElementById('chat-list').innerHTML;
...
function sendMsg(sendMsg) {
    ...
    let msg = (typeof(sendMsg) === 'undefined') ? document.getElementById('msg').value : sendMsg;
    if ('' === msg) {
        return false;
    }
    ...
    let _html = document.getElementById('chat-list').innerHTML;
    let time = getTime();
    let content = replaceContent(msg);
    ...
    socket.send(JSON.stringify({
        type: 'chatMessage',
        data: {
            to_id: kf_id, to_name: kf_name, content: msg, from_name: config.name,
            from_id: config.uid, from_avatar: config.avatar
        }
    }));

    // 储存我发出的信息
    let key = kf_id + '-' + config.uid;
    if (typeof(Storage) !== "undefined") {
        let localMsg = getCache(key);
...
function showMsg(info, flag) {
    ...
    let _html = document.getElementById('chat-list').innerHTML;
    let content = replaceContent(info.content);
...
function showChatLog() {
    ...
    let chatLog = getCache(kf_id + '-' + config.uid);
    if (chatLog == null || chatLog.length === 0) {
        return;
    }

    let _html = '';
    let len = chatLog.length;
    for (let i = 0; i < len; i++) {
        let item = chatLog[i];
...
// 对话框定位到最底端
function wordBottom() {
    let ex = document.getElementById("chat-list");
