...
// 创建一个Socket实例
let socket = new WebSocket('ws://' + socket_server);
...
socket.onopen = function (res) {
    ...
    // 登录
    let login_data = '{"type":"init", "uid":"' + uinfo.id + '", "name" : "' + uinfo.username + '", "avatar" : "'
...
$(function () {
    ...
    $.getJSON('/service/index/getUserList', function (res) {
        if (1 == res.code && res.data.length > 0) {
            ...
            let id = $(".layui-unselect").find('li').eq(0).data('id');
            let name = $(".layui-unselect").find('li').eq(0).data('name');
            let avatar = $(".layui-unselect").find('li').eq(0).data('avatar');
            let ip = $(".layui-unselect").find('li').eq(0).data('ip');
    ...
    $("#face").click(function (e) {
        ...
        layui.use(['layer'], function () {
            ...
            let isShow = $(".layui-whisper-face").css('display');
            if ('block' === isShow) {
                layer.close(index);
                return;
            }
            let height = $(".chat-box").height() - 110;
            layer.ready(function () {
                index = layer.open({
                    type: 1,
    ...
    $('#end-ask').click(function () {
        ...
        layerIndex = layer.confirm('确定要结束会诊吗?', {
            ...
            let uid = $("#active-user").attr('data-id');
    ...
    $("#scroll-link").click(function () {
        let id = $("#active-user").attr('data-id');
        let name = $("#active-user").attr('data-name');
        let avatar = $("#active-user").attr('data-avatar');
        let ip = $("#active-user").attr('data-ip');
            ...
            layui.use(['form'], function () {
                ...
                form.on('select(group)', function (data) {
                    if (uinfo.group === data.value) {
                        ...
                        layer.close(layerIndex);
                        let group = data.value; // 分组
                        // 交换分组
                        let change_data = '{"type":"changeGroup", "uid":"' + id + '", "name" : "' + name + '", "avatar" : "'
...
layui.use(['upload', 'layer'], function () {
    ...
    // 执行实例
    let uploadInstImg = upload.render({
        elem: '#image' // 绑定元素
        , accept: 'images'
    ...
    let uploadInstFile = upload.render({
        elem: '#file' // 绑定元素
        , accept: 'file'
        , exts: 'zip|rar'
...
function showFaces() {
    isShow = true;
    let alt = getFacesIcon();
    let _html = '<div class="layui-whisper-face"><ul class="layui-clear whisper-face-list">';
...
function checkFace(obj) {
    let word = $(".msg-area").val() + ' face' + $(obj).attr('title') + ' ';
    $(".msg-area").val(word).focus();
}

// 发送消息
function sendMessage(sendMsg) {
    let msg = (typeof(sendMsg) === 'undefined') ? $(".msg-area").val() : sendMsg;
    if ('' === msg) {
        layui.use(['layer'], function () {
            let layer = layui.layer;
    ...
    let word = msgFactory(msg, 'mine', uinfo);
    let uid = $("#active-user").attr('data-id');
    let uname = $("#active-user").attr('data-name');
...
function addUser(data) {
    ...
    let _html = '<li class="layui-nav-item" data-id="' + data.id + '" id="f-' + data.id +
    ...
    $.getJSON('/service/index/getChatLog', {uid: uid, page: page}, function (res) {
        if (1 === res.code && res.data.length > 0) {
            ...
            if (res.msg === res.total) {
                let _html = '<div class="layui-flow-more">没有更多了</div>';
            } else {
                let _html = '<div class="layui-flow-more"><a href="javascript:;" data-page="' + parseInt(res.msg + 1)
...
function getMore(obj) {
    ...
    let uid = $(".layui-unselect").find('li').eq(0).data('id');
...
function loginOut() {
    ...
    $("#user_list li").each(function () {
        ...
        let uid = $(this).data('id');
        let activeUid = $("#active-user").attr('data-id');
        if (uid === activeUid) {
