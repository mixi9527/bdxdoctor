...
class Events
{
    ...
    {
        if (empty(self::$db)) {
            self::$db = new \Workerman\MySQL\Connection('127.0.0.1', '3306', 'root', '123456', 'bdxdoctor');
        }
        ...
        switch ($message['type']) {
            ...
            case 'chatMessage':
                ...
                if (!empty($client)) {
                    ...
                    self::$db->insert('fa_chat_log')->cols($serviceLog)->query();
                    unset($serviceLog);
                }
                break;
            // 转接
            case 'changeGroup':
                ...
                // 记录该客服与该会员的服务结束时间
                self::$db->query("update `fa_service_log` set `end_time` = " . time() . " where `client_id`= '" . $userClient . "'");
        ...
        foreach ($kfList as $k => $v) {
            foreach ($v as $key => $vo) {
                if (in_array($client_id, $vo['user_info'])) {
                    ...
                    self::$db->query("update `fa_service_log` set `end_time` = " . time() . " where `client_id`= '" . $client_id . "'");
                    ...
                    //$outUser = self::$db->query("select `user_id`,`group_id` from `fa_service_log` where `client_id`= '" . $client_id . "'");
                    // 通知 客服删除退出的用户
                    if (!empty($outUser)) {
                        $del_message = [
    ...
    private static function userOnlineTask($client_id, $group)
    {
        ...
        if (1 == $res['code']) {
            ...
            // 检测是否开启自动应答
            $sayHello = self::$db->query('select `word`,`status` from `fa_reply` where `id` = 1');
            ...
            self::$db->insert('fa_service_log')->cols($serviceLog)->query();
            unset($serviceLog);
    ...
    {
        $maxNumber = self::$db->query('select `max_service` from `fa_kf_config` where `id` = 1');
        if (!empty($maxNumber)) {
            $maxNumber = 5;
        } else {
    ...
    private static function writeLog($flag = 1)
    {
        ...
        self::$db->update('fa_now_data')->cols($param)->where('id=1')->query();
        ...
        if (2 == $flag) {
            ...
            self::$db->insert('fa_service_data')->cols($param)->query();
        }
